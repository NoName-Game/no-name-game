...
func GetCurrentControllerCache(playerID uint32) (result string, err error) {
	if result, err = services.Redis.Get(fmt.Sprintf("player_%v_current_controller", playerID)).Result(); err != nil {
...
func SetCurrentControllerCache(playerID uint32, controller string) {
	if err := services.Redis.Set(fmt.Sprintf("player_%v_current_controller", playerID), controller, 0).Err(); err != nil {
		panic(err)
	}
}
...
func DelCurrentControllerCache(playerID uint32) {
	if err := services.Redis.Del(fmt.Sprintf("player_%v_current_controller", playerID)).Err(); err != nil {
		panic(err)
	}
}
...
func SetControllerCacheData(playerID uint32, controller string, stage int32, payload interface{}) (err error) {
	...
	return services.Redis.Set(fmt.Sprintf("player_%v_controller_%s", playerID, controller), marshalData, 60*time.Minute).Err()
}

func GetControllerCacheData(playerID uint32, controller string, payload interface{}) (stage int32, err error) {
	var result string
	result, _ = services.Redis.Get(fmt.Sprintf("player_%v_controller_%s", playerID, controller)).Result()
