...
// Crafting
func Crafting(update tgbotapi.Update, player nnsdk.Player) {
	//====================================
	// Init Func!
	//====================================
	...
	case 2:
		...
		inventory, err := provider.GetPlayerInventory(player)
		if err != nil {
		...
		for r, q := range playerResources {
			...
			if payload.Resources[r] < q {
				...
				keyboardRow := tgbotapi.NewKeyboardButtonRow(tgbotapi.NewKeyboardButton(
					helpers.Trans("crafting.add", player.Language.Slug) + " " + resource.Name + " (" + (strconv.Itoa(q - payload.Resources[r])) + ")",
				))
	...
	case 4:
		if validationFlag {
			...
			for k, q := range payload.Resources {
				_, err := provider.RemoveResourceToPlayerInventory(player, nnsdk.AddResourceRequest{
					ItemID:   k,
					Quantity: q,
				})
